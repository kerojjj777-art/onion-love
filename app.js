            ...
            <div class="catalog-grid" style="display: flex; flex-direction: column; gap: 10px;">
                ...
                <div class="catalog-item" onclick="window.devMigrateLegacyRoomDataToOnionGang()" style="flex-direction:column; justify-content:center; padding: 15px; font-weight:bold; background:rgba(217, 83, 79, 0.12); border-color:#d9534f; width:100%; box-sizing:border-box;">
                    ...
                    <small style="margin-top:6px; color:#d9534f; font-size:11px;">臨時一次性按鈕，成功後請刪除</small>
                                ...
                                <div class="catalog-item" onclick="window.devMigrateLegacyChatsToOnionGang()" style="flex-direction:column; justify-content:center; padding: 15px; font-weight:bold; background:rgba(0, 170, 255, 0.12); border-color:#00aaff; width:100%; box-sizing:border-box;">
...
// ====== 臨時一次性工具：舊全域 chats 遷移到洋蔥五告派 ======
// 注意：這只搬公開大廳對話 chats，不搬 privateChats 私訊。
// 遷移成功並完成測試後，請移除此整段與 dev-modal 按鈕，避免正式版誤按。
window.devMigrateLegacyChatsToOnionGang = async function() {
    if (!window.GameLogic || !window.GameLogic.currentUser) {
        alert("請先登入管理員帳號。");
        return false;
    }
    ...
    if (!isAdmin) {
        alert("只有管理員可以執行舊對話遷移。");
        console.warn("[舊對話遷移] 非管理員嘗試執行：", user.email, user.uid);
        return false;
    }
    ...
    const firstConfirm = confirm(
        "這是一次性舊對話遷移工具。\n\n" +
        "將只複製：\n" +
        "chats → serverRooms/onionGang/chats\n\n" +
        "不會刪除舊 chats。\n" +
        "不會搬 privateChats 私訊。\n" +
        "不會搬 users、manuals、onlinePlayers、cafePlayers。\n" +
        "不會寫入 ryoFriends。\n\n" +
        "如果 onionGang/chats 已有新對話，會保留新對話，只補上舊資料中尚未存在的 key。\n\n" +
        "確定要開始檢查並準備遷移嗎？"
    );

    if (!firstConfirm) return false;

    const sourcePath = "chats";
    const targetPath = "serverRooms/onionGang/chats";

    try {
        const [legacyChatsSnap, targetChatsSnap] = await Promise.all([
            get(ref(window.GameLogic.db, sourcePath)),
            get(ref(window.GameLogic.db, targetPath))
        ]);

        const legacyChats = legacyChatsSnap.val();
        const targetChats = targetChatsSnap.val() || {};

        if (!legacyChats || typeof legacyChats !== "object" || Object.keys(legacyChats).length === 0) {
            alert("舊全域 chats 沒有可遷移的資料。");
            console.warn("[舊對話遷移] 舊全域 chats 為空。");
            return false;
        }

        const legacyKeys = Object.keys(legacyChats);
        const targetHasData = targetChats && typeof targetChats === "object" && Object.keys(targetChats).length > 0;

        if (targetHasData) {
            const proceed = confirm(
                "偵測到 serverRooms/onionGang/chats 已經有資料。\n\n" +
                "本工具會採用安全補寫模式：\n" +
                "1. 只寫入 onionGang 尚未存在的舊 chats key。\n" +
                "2. 已存在的同名 key 會略過，不覆蓋。\n" +
                "3. onionGang 目前的新對話會保留。\n\n" +
                "是否繼續？"
            );
        ...
        legacyKeys.forEach(key => {
            if (
                targetChats &&
                typeof targetChats === "object" &&
                Object.prototype.hasOwnProperty.call(targetChats, key)
            ) {
                skippedExisting.push(key);
                return;
            }

            updates[`${targetPath}/${key}`] = legacyChats[key];
        });
        ...
        if (writeCount === 0) {
            alert(
                "沒有需要補寫的舊對話。\n\n" +
                `舊 chats 筆數：${legacyKeys.length}\n` +
                `已存在而略過：${skippedExisting.length}\n\n` +
                "可能先前已經搬過，或 onionGang/chats 已經有相同資料。"
            );
            console.warn("[舊對話遷移] 沒有可寫入資料。", { legacyKeys, skippedExisting });
            return false;
        }
        ...
        alert(
            "舊大廳對話遷移完成！\n\n" +
            `舊 chats 總筆數：約 ${legacyKeys.length}\n` +
            `本次補寫到 onionGang/chats：${writeCount}\n` +
            `同名已存在而略過：${skippedExisting.length}\n\n` +
            "舊全域 chats 沒有被刪除。\n" +
            "privateChats 私訊沒有被搬移。\n" +
            "ryoFriends 沒有被寫入。\n\n" +
            "測試完成後，請移除此一次性遷移函式與按鈕。"
        );

        console.log("[舊對話遷移] 完成。", {
            sourcePath,
            targetPath,
        ...
        if (typeof listenToChat === "function") {
            listenToChat();
        }

        return true;
    } catch (err) {
        console.warn("[舊對話遷移] 執行失敗：", err);
        alert("舊對話遷移失敗，請打開 F12 Console 查看錯誤。\n\n" + (err && err.message ? err.message : err));
        return false;
    }
};
